...
OLLAMA_URL = "http://localhost:11434/api/generate"  # Локальный Ollama API
MODEL = "tinyllama"  # Легкая модель для быстрой работы
...
class CodeApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.debugger.bind("<Button-1>", lambda event: self.debugger.focus_set())
    ...
    def run_ai_query(self, prompt):
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                update_output("Убедитесь, что Ollama запущен локально (ollama serve)\n")
                return

            try:
                response_json = response.json()
                generated_text = response_json.get('response', '')
                cleaned_text = process_content(generated_text)
                update_output(cleaned_text + "\n")
            except (KeyError, json.JSONDecodeError) as e:
                update_output(f"\nОшибка обработки ответа: {e}\n")
                print(f"Неверный формат ответа: {response.text}")

        except requests.exceptions.ConnectionError:
            update_output("\nОшибка подключения к Ollama. Убедитесь, что:\n")
            update_output("1. Ollama установлен (https://ollama.ai)\n")
            update_output("2. Сервер запущен: ollama serve\n")
            update_output(f"3. Модель установлена: ollama pull {MODEL}\n")
        except Exception as e:
            update_output(f"\nОшибка сети: {e}\n")
    ...
    def new_file(self):
        ...
        input_text.bind("<KeyRelease>",
                        lambda e: [self.update_line_numbers_tab(input_text, line_numbers), self.highlight_syntax()])
        ...
        input_text.bind("<Up>", lambda e: self.update_line_numbers_tab(input_text, line_numbers))
        input_text.bind("<Down>", lambda e: self.update_line_numbers_tab(input_text, line_numbers))
